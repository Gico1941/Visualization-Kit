...
GSEA_bubble <- function(GSEA_folder='GSEA',GSEA_fdr_hold=0.5,fdr_top=20){
  ...
  files <- list.files(GSEA_folder,pattern='_report_for',recursive = T,full.names = T)
    ...
    plot <- function(dt=data[[1]]){
      ...
      dt$NAME <- factor(dt$NAME,levels = dt$NAME[order(dt$NES)])
          ...
          ggplot(dt,aes(x=NES,y=NAME,size=SIZE,color=`FDR.q.val`))+
            ...
            ylab(NULL)+
            xlab('Absolute NES')+
            ggtitle(paste0(tail(strsplit(basename(fl),'_')[[1]],2),'_GSEA_enrichment_',group))+
            theme(axis.text.y=element_text(size=10))
 
      ggsave(paste0(dirname(fl),'_',group,'.pdf'),height = max(7,0.5*nrow(dt)))
    }
...
#GSEA_bubble(GSEA_folder='GSEA',GSEA_fdr_hold=0.5,fdr_top=20)   ### GSEA
